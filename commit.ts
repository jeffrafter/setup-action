...
import {GitCreateTreeParamsTree, GitCreateRefResponse} from '@octokit/rest'

export interface CommitParams {
  owner: string
  repo: string
  files: Array<CommitFile>
  baseRef: string // heads/master
  headRef: string // refs/heads/branch
}
...
const commit = async (octokit: github.GitHub, params: CommitParams): Promise<GitCreateRefResponse> => {
  ...
  const paths: Array<string> = baseTreeResponse.data.tree.map((item: CommitTreeEntry) => {
    return item.path
  })
  console.log({paths})
